    ...
    $.each(
      ...
      function (mapId, map) {
        ...
        if ($(map.container).length >= 1
          ...
          // If field values already set, set marker.
          if (
            !isNaN(parseFloat($('.canvas-' + mapId + ' .geolocation-hidden-lat').attr('value')))
            && !isNaN(parseFloat($('.canvas-' + mapId + ' .geolocation-hidden-lng').attr('value')))
          ) {
